...
export class GraphQLRequestVisitor extends ClientSideBaseVisitor<RawGraphQLRequestPluginConfig, GraphQLRequestPluginConfig> {
  ...
  constructor(schema: GraphQLSchema, fragments: LoadedFragment[], rawConfig: RawGraphQLRequestPluginConfig) {
    ...
    this._additionalImports.push(`import { GraphQLClient } from '@ninetailed/cf-worker-graphql-request';`);
    ...
    if (this.config.rawRequest) {
      this._additionalImports.push(`import { GraphQLError } from '@ninetailed/cf-worker-graphql-request/dist/src/types';`);
    }
  }
  ...
  public get sdkContent(): string {
    const allPossibleActions = this._operationsToInclude
      .map(o => {
        ...
        if (this.config.rawRequest) {
          return `${o.node.name.value}(variables${optionalVariables ? '?' : ''}: ${o.operationVariablesTypes}, event?: FetchEvent, options: {
            cache: boolean;
            cacheKey?: string;
            cacheTtl?: number;
          } = {
            cache: false,
          }): Promise<{ data?: ${o.operationResultType} | undefined; extensions?: any; headers: Headers; status: number; errors?: GraphQLError[] | undefined; }> {
    return client.rawRequest<${o.operationResultType}>(${doc}, variables, event, options);
}`;
        } else {
          return `${o.node.name.value}(variables${optionalVariables ? '?' : ''}: ${o.operationVariablesTypes}, event?: FetchEvent, options: {
            cache: boolean;
  ...
  return client.request<${o.operationResultType}>(${doc}, variables, event, options);
}`;
        }
      })
